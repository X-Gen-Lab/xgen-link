...
| Wire | `src/wire` | `xgl_wire.h`, `xgl_frame.h`, `xgl_parser.h` | `xgl_wire.c`, `xgl_frame.c`, `xgl_parser.c`, `xgl_crc.c` | Wire encoding is offset-based, not packed-struct based |
| Security | `src/security` | `xgl_security.h` | `xgl_security.c` | Replay windows are scoped by source, connection, session, and packet |
| Datalink | `src/datalink` | `xgl_datalink.h` | `xgl_datalink.c` | Frames that fail CRC/auth/replay do not enter network |
| Network | `src/network` | `xgl_network.h`, `xgl_route.h` | `xgl_network.c`, `xgl_route.c` | Routing, TTL, MTU, and forwarding are resolved here |
| Transport | `src/transport` | `xgl_transport.h`, `xgl_reliable.h`, `xgl_window.h`, `xgl_fragment.h`, `xgl_rtt.h` | `xgl_transport.c`, `xgl_reliable.c`, `xgl_window.c`, `xgl_fragment.c`, `xgl_rtt.c` | Reliable state is scoped by peer key and delivered in order |
...
| Peer/packet number | `src/transport/xgl_transport.c` | Create scoped peer state and assign 32-bit packet numbers | peer allocation failure, full window |
...
| Reliability | `src/transport/xgl_transport.c` | Process ACK/SACK, buffer out-of-order packets, filter duplicates | wrong connection/session does not pollute other peers |
...
Wire, parser, reliable, window, and fragment headers are for protocol maintenance, tests, or advanced integrations. They should not be documented as stable user ABI.
...
- Wire field changes must update `xgl_wire.h`, `xgl_wire.c`, wire format docs, and offset tests.
