...
| 分片 | `src/transport/xgl_transport_send_plan.c`, `src/transport/xgl_transport_send_fragment.c`, `src/transport/xgl_fragment.c` | 规划分片预算、发送 `FRAGMENT_EXT` 并重组超 MTU payload | message 过大、预算不足 |
...
| local or forward | `src/network/xgl_network_receive.c` | 本地交付，或 TTL 递减、重算 CRC 后转发 | TTL/route/MTU 失败则丢弃 |
