...
| Wire | `src/wire` | `xgl_wire.h`, `xgl_frame.h`, `xgl_parser.h` | `xgl_wire.c`, `xgl_frame.c`, `xgl_parser.c`, `xgl_crc.c` | wire 编解码按 offset 手写，不依赖 packed struct |
| Security | `src/security` | `xgl_security.h` | `xgl_security.c` | replay window 按 source、connection、session、packet 隔离 |
| Datalink | `src/datalink` | `xgl_datalink.h` | `xgl_datalink.c` | 未通过 CRC/auth/replay 的 frame 不进入 network |
| Network | `src/network` | `xgl_network.h`, `xgl_route.h` | `xgl_network.c`, `xgl_route.c` | route、TTL、MTU 和 forwarding 在 network 层闭环 |
| Transport | `src/transport` | `xgl_transport.h`, `xgl_reliable.h`, `xgl_window.h`, `xgl_fragment.h`, `xgl_rtt.h` | `xgl_transport.c`, `xgl_reliable.c`, `xgl_window.c`, `xgl_fragment.c`, `xgl_rtt.c` | 可靠状态按 peer key 隔离，有序交付给应用 |
...
| peer/packet number | `src/transport/xgl_transport.c` | 按 peer key 创建状态，分配 32-bit packet number | peer pool/allocator 失败、窗口满 |
...
| route lookup | `src/network/xgl_network.c`, `src/network/xgl_route.c` | 查找目标 route，检查 route MTU | 无路由、MTU 不足、TTL 无效 |
| frame build | `src/wire/xgl_frame.c` | 构造 v2 header、TLV、payload、CRC/auth trailer | header/ext 长度越界、auth provider 缺失 |
| PHY send | `src/datalink/xgl_datalink.c` | 发送 serialized frame | PHY 返回错误 |
...
| reliability | `src/transport/xgl_transport.c` | 处理 ACK/SACK、乱序缓存、重复包过滤 | 错误 connection/session 不污染其他 peer |
...
协议维护、测试和高级集成才需要 wire/parser/reliable/window/fragment 等头。文档和安装规则应避免把内部状态结构描述成稳定 ABI。
...
- 修改 wire 字段时，同步更新 `xgl_wire.h`、`xgl_wire.c`、wire format 文档和 offset 测试。
